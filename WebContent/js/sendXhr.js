...
function goPage(currentPage, requestType) {
	ajax('get','RequestHandler?type='+requestType+'&page='+currentPage, null, function(pageAttr) {
		...
		pageNav.innerHTML = "<li><a href='javascript:goPage(1,'keyword')'>首页</a></li>" + 
												"<li><a href='javascript:goPage("+(pageAttr.currentPage - 1)+", 'keyword')'>上一页</a></li>" +
												pageAttr.currentPage + " / " + pageAttr.totalPage + 
												"<li><a href='javascript:goPage("+(pageAttr.currentPage + 1)+", 'keyword')'>下一页</a></li>" + 
												"<li><a href='javascript:goPage("+pageAttr.totalPage+", 'keyword')'>尾页</a></li>";
	});
}